...
\hypersetup{
  ...
  linkcolor=black,
  urlcolor=blue,
  pdftitle={myanmar Package Manual},
...
% Title info
% --------------------------------------------------
\title{\textbf{The \pkg{myanmar} Package}\\
Unicode Myanmar Script Support for LaTeX\\[4pt]
...
The \pkg{myanmar} package provides a compact interface for typesetting
Myanmar-script text in LaTeX documents. It is designed for Unicode input and
OpenType fonts, and therefore works with XeLaTeX and LuaLaTeX only.
...
\begin{burmese}
ဒီလမ်းညွှန်က \texttt{myanmar.sty} ထဲမှာပါတဲ့ အသုံးပြုသူတွေအတွက် command တွေနဲ့ 
...
The following LaTeX packages are loaded by \pkg{myanmar}:
...
\texttt{myanmar.ins}. To generate \texttt{myanmar.sty}, run:
...
\texttt{myanmar.ins} ပါ။ \texttt{myanmar.sty} ကို ထုတ်ယူဖို့ အပေါ်က 
command ကို run ပေးပါ။
\end{burmese}

For local use, place the generated \texttt{myanmar.sty} beside your document or
compile from the package root directory.

\begin{burmese}
Local document မှာ သုံးချင်ရင်တော့ ထွက်လာတဲ့ \texttt{myanmar.sty} ကို 
ကိုယ့်ရဲ့ \texttt{.tex} file နဲ့ folder အတူတူ ထားပေးပါ။ ဒါမှမဟုတ်လည်း package ရဲ့ root 
directory ကနေ compile လုပ်လို့ရပါတယ်။
\end{burmese}

For a user-level installation, place \texttt{myanmar.sty} in a local TeX tree,
for example:
...
\begin{burmese}
User-level installation အတွက်ဆိုရင်တော့ \texttt{myanmar.sty} ကို local TeX tree 
...
\opt{language=shan} & Shan \\
\opt{language=mon} & Mon \\
\opt{language=sgawkaren} & S'gaw Karen \\
\bottomrule
\end{tabular}
\end{center}
...
This repository is distributed under the MIT License. See the \texttt{LICENSE}
file for the full license text.

\begin{burmese}
ဒီ repository ကို MIT License နဲ့ ဖြန့်ဝေပေးထားပါတယ်။ License အပြည့်အစုံကိုတော့ 
\texttt{LICENSE} file ထဲမှာ ဝင်ဖတ်လို့ရပါတယ်။
\end{burmese}
